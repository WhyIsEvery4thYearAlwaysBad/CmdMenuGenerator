		...
		for (std::size_t i=0; i < p_sInStr.length(); )
		{
			if (bInBlockComment) {
				if (p_sInStr.find("*/",i)==std::string::npos) {
					TokenContainer.push_back(Token(iLineNum,iLineColumn,TokenType::COMPILER_ERROR,std::to_string(iLineNum)+':'+std::to_string(iLineColumn)+": error: Block quote has no ending sequence ('*/')."));
					break;
				}
			...
			case '\"':
				...
				for (i++, iLineColumn++; i < p_sInStr.length(); i++, iLineColumn++) {
					if (p_sInStr.at(i)=='\r') {
						TokenContainer.push_back(Token(iLineNum,iLineColumn,TokenType::COMPILER_ERROR,std::to_string(iLineNum)+':'+std::to_string(iLineColumn)+": error: Ending '\"' is missing."));
						// 
